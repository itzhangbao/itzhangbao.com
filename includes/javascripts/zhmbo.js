/*
 *  zhmbo-official-website 1.0.0
 *  
 *  Hi, I`m zhmbo, a Developer from 深圳
 *  https://zhmbo.cn/
 *  hi@zhmbo.cn
 *  
 *  Last update on: 2024/7/31
 *  ©2024 zhmbo. all rights reserved.
 */
...
var Server = {
    ...
    SERVER_URL: "https://MdYXbMMI.api.lncldglobal.com",
    init: function () {
        AV.init({
            appId: Server.APP_ID,
            appKey: Server.APP_KEY,
            serverURL: Server.SERVER_URL // 国际版url可忽略
        });
    },
...
var Submit = {
  ...
  push: function (form) {
    ...
    Server.save(message).then(function (object) {
      // 成功保存之后，执行其他逻辑
...
Router.routes = function (callback) {
  ...
  //  SECRET
  else if (location[0] == 'secret' && location[1] === undefined) Router.push('secret');
    ...
    //  BUCKET
      ...
      //  BUCKET ALL
        ...
        //  PROJECTS
          ...
          //  BLOG
          else if (location[0] == 'blog' && location[1] === undefined) Router.push('blog');

            //  ABOUTME
            else if (location[0] == 'aboutme' && location[1] === undefined) Router.push('aboutme');
...
Router.callbacks = function (wrapper) {
  ...
  if (wrapper == 'secret') secret();else if (wrapper == 'notFound') notFound();else if (wrapper == 'home') home();else if (wrapper == 'blog') blog();
};
...
var upTime = '2024-07-31 16:38:16';
...
function blog() {
    setTimeout(function () {
        Timer.run('.template[data-template=blog] time', function () {
